...
class SessionRecorder {
  ...
  private lastEngagementSave: number = 0
  ...
  start() {
    ...
    // Generate session ID (potentially using Amplitude)
    this.sessionId = this.generateSessionId()
    ...
    // Save frequently for smooth video (back to 2 seconds)
  ...
  async saveRecording() {
    ...
    try {
      // Strict port assignment based on version
      const targetPort = this.version === 'A' ? '3001' : '3002'
      let saved = false
