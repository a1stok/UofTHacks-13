...
function getRecordingsDir(): string {
  const possiblePaths = [
    // Running from test-website folder: ../output/recordings-b
    path.join(process.cwd(), '..', 'output', 'recordings-b'),
    // Running from monorepo root via turbo: output/recordings-b  
    path.join(process.cwd(), 'output', 'recordings-b'),
    // Explicit path as fallback
    path.resolve('C:/Users/Kostiantyn/Startup/output/recordings-b'),
  ]
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Format: B_2026-01-18T05-20-09-666Z_session_xxx.json
